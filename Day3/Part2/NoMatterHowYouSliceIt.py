    ...
    def __iadd__(self, pattern):
        ...
        for w in range(pattern.width):
            for h in range(pattern.height):
                ...
                if self.surface[y][x] == '.':
                    self.surface[y][x] = '0'
                    self.pixel_owners[f"({x},{y})"] = set()
                self.surface[y][x] = str(int(self.surface[y][x]) + 1)
                self.pixel_owners[f"({x},{y})"].add(pattern.pattern_id)
        return self
...
class Pattern:
    ...
    def parse_desc_str(self):
        ...
        offsets, dimensions = map(str.strip, square_info.split(':'))
...
def main():
    ...
    for ds in desc_strs:
        pattern = Pattern(ds)
        all_patterns.add(pattern.pattern_id)
        square += pattern
    ...
    for owner in square.pixel_owners:
        if len(square.pixel_owners[owner]) > 1:
            all_patterns -= square.pixel_owners[owner]
